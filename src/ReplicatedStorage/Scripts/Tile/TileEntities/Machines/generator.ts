...
class Generator extends TileEntity {
    ...
    static decode(decoded: unknown, gridBase: BasePart): Generator {
        ...
        const generator = new Generator(data.name, decodeVector3(data.position), decodeVector2(data.size), decodeVector2(data.direction), gridBase as BasePart, data.speed);
